...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc +4; printf("imm = %020ld\n", imm); s->dnpc = s->pc+imm;);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc +4; s->dnpc = (src1 + imm ) & (~1));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = (src1 + imm) & 0xFFFF);
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw   , R, R(dest) = (SEXT(BITS(src1 + src2, 31, 0), 64)); printf("src1 = %064ld\nsrc2 = %064ld\nrd =   %064ld\n", src1, src2, R(dest)));
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(dest) = src1 - src2);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = src1 < imm ? 1 : 0);
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1 == src2) s->dnpc = (s->pc + imm);printf("src1 = %064ld\nsrc2 = %064ld\n", src1, src2));
