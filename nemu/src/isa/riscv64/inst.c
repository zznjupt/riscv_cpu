...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if((uint64_t)src1 >= (uint64_t)src2) s->dnpc = s->pc + imm); // unsigned 
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(dest) = src1 << SEXT(BITS(src2, 5, 0), 64)); //
