...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, printf("imm = %ld\n", imm);Mw(src1 + imm, 1, (word_t)BITS(src2, 7, 0)););
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, printf("src1 = %016lx\n", src1);R(dest) = src1 << SEXT(BITS(imm, 5, 0), 64);printf("imm  = %016lx\n", imm);printf("rd   = %016lx\n", R(dest))); //
