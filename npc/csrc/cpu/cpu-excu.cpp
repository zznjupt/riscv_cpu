...
int excu_once(int exit) {
	contextp->timeInc(1);
	if ( contextp->time()      > RstTime)   top->rst = 0
	if ((contextp->time() % 2) == 1)        top->clk = 1;
	else if ((contextp->time() % 2) == 0)   top->clk = 0;
	top->eval();
	if (top->clk == 1) {
#ifdef ITRACE
		int inst;
	...
	if (top->instaddr_o >= 0x0000000080000000 && top->rst == 0) {
