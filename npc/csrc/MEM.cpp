...
extern "C" void MEM_pmem_write(uint64_t waddr, uint64_t wdata, uint8_t wmask, bool w_en) {
    ...
    if(waddr >= CONFIG_MBASE) {
        ...
        printf("cprintf: MEM stage write in \33[1;34m0x%016lx\33[0m\nwdata = \33[1;34m0x%016lx\33[0m, wmask = 0x%02x\n", waddr, wdata, wmask);
        switch (length) {
...
extern "C" void MEM_pmem_read(uint64_t raddr, uint64_t* rdata, bool r_en) {
    ...
    if(raddr >= CONFIG_MBASE) {
        *rdata = *(uint64_t*) guest_to_host(raddr);
        printf("cprintf: MEM stage read from \33[1;34m0x%016lx\33[0m, rdata = \33[1;34m0x%016lx\33[0m\n", raddr, *rdata);
    } else assert(0);
}
